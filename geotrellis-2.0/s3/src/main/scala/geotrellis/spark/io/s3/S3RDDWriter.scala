...
trait S3RDDWriter {
  ...
  )(useHadoopConfS3: Boolean = false): Unit = {
    update(rdd, bucket, keyPath, None, None, putObjectModifier, threads)(true)
  }
  ...
  private[s3] def update[K: AvroRecordCodec: ClassTag, V: AvroRecordCodec: ClassTag](
    ...
    val _getS3Client = if (useHadoopConfS3) {
      () => S3Client.awsClientFromConf(S3Client.hadoopConf2Map(sc.hadoopConfiguration))
    } else
      getS3Client
