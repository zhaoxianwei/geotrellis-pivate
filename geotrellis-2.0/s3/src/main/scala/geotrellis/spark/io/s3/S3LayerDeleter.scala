...
class S3LayerDeleter(val attributeStore: AttributeStore, client: Option[S3Client] = None) extends LazyLogging with LayerDeleter[LayerId] {

  def getS3Client = {
    if (attributeStore.isInstanceOf[S3AttributeStore]) {
      attributeStore.asInstanceOf[S3AttributeStore].s3Client
    } else {
      client.getOrElse(S3Client.DEFAULT)
    }
  }
...
object S3LayerDeleter {
  ...
  def apply(attributeStore: AttributeStore, s3Client: S3Client): S3LayerDeleter =
    new S3LayerDeleter(attributeStore, Some(s3Client))
}
