...
object S3LayerReindexer {
  ...
  def apply(attributeStore: AttributeStore, bucket: String, prefix: String, s3Client: S3Client)(implicit sc: SparkContext): LayerReindexer[LayerId] = {
    val layerReader  = S3LayerReader(attributeStore)
    val layerWriter  = S3LayerWriter(attributeStore, bucket, prefix)
    val layerDeleter = S3LayerDeleter(attributeStore)
    val layerCopier  = S3LayerCopierV2(attributeStore, s3Client)
