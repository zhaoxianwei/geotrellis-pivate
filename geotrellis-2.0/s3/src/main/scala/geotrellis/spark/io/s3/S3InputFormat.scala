...
object S3InputFormat {
  ...
  // Add this to pass ak/sk parameter for obs in huawei cloud
  var conf: Option[Map[String, String]] = None
  ...
  def setOtherConf(otherConf: Option[Map[String, String]]): Unit =
    conf = otherConf
  ...
  def getS3Client(job: JobContext): S3Client =
    job.getConfiguration.getSerializedOption[() => S3Client](CREATE_S3CLIENT) match {
      ...
      case None => conf.fold(S3Client.hadoopDefaultAwsClient(job.getConfiguration))(S3Client.awsClientFromConf)
    }
