...
trait ConfigFormats {
  ...
  case class OutputFormat(bp: Map[String, BackendProfile]) extends RootJsonFormat[Output] {
    ...
    def write(o: Output): JsValue = JsObject(
      ...
      "globalIndex"         -> o.globalIndex.toJson,
      "update"              -> o.update.toJson,
      ...
      value match {
        case JsObject(fields) =>
          Output(
            ...
            globalIndex         = fields.get("globalIndex").map(_.convertTo[Boolean]).fold(false)(identity),
            update              = fields.get("update").map(_.convertTo[Boolean]).fold(false)(identity),
